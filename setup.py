#!/usr/bin/env python3
"""Odysseus Lite — first-time setup script.
...
def reset_password(argv):
    """`python setup.py reset-password [username]` — operator recovery path.

    Sets a new password for an existing user (default: the first admin) without
    needing the old one, so a missed temp admin password does NOT require wiping
    the database. Reads the new password from --password, the
    ODYSSEUS_RESET_PASSWORD env var, or a hidden interactive prompt; if none is
    given, generates a strong random one and prints it once.
    """
    import argparse
    import getpass
    import secrets

    parser = argparse.ArgumentParser(prog="setup.py reset-password")
    parser.add_argument("username", nargs="?", default=None,
                        help="user to reset (default: first admin)")
    parser.add_argument("--password", default=None, help="new password (else prompt/random)")
    args = parser.parse_args(argv)

    try:
        from core.auth import AuthManager
    except Exception as e:
        print(f"[error] could not load AuthManager: {e}")
        return 1

    auth = AuthManager()
    users = auth.users or {}
    if not users:
        print("[error] no users exist yet — run plain `python setup.py` to create the admin.")
        return 1

    username = args.username
    if not username:
        admins = [u for u, v in users.items() if v.get("is_admin")]
        username = (admins or list(users))[0]
    username = username.strip().lower()
    if username not in users:
        print(f"[error] user '{username}' not found. Known users: {', '.join(users)}")
        return 1

    new_password = args.password or os.getenv("ODYSSEUS_RESET_PASSWORD")
    generated = False
    if not new_password:
        if sys.stdin.isatty():
            new_password = getpass.getpass(f"New password for '{username}': ").strip()
    ...
    if not auth.admin_reset_password(username, new_password):
    ...
    if generated:
        ...
        print("  Log in and change it under Settings.")
    print()
    return 0


if __name__ == "__main__":
    if len(sys.argv) > 1 and sys.argv[1] in ("reset-password", "reset_password"):
        raise SystemExit(reset_password(sys.argv[2:]))
    main()
