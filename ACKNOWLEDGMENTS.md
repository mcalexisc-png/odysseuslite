...
Odysseus Lite stands on the shoulders of a lot of open-source work. This file
...
**Odysseus Lite is a fork of [Odysseus](https://github.com/pewdiepie-archdaemon/odysseus)**,
created by PewDiePie and contributors. Copyright © the Odysseus authors.
Upstream's [`LICENSE`](https://github.com/pewdiepie-archdaemon/odysseus/blob/main/LICENSE)
file and its `README.md` ("License: AGPL-3.0-or-later") both identify the
project as **AGPL-3.0-or-later**; GitHub's own license detector agrees
(`license.spdx_id: AGPL-3.0`). This fork keeps the same license — see
[LICENSE](LICENSE). This fork vendors the upstream implementation and adds a
CPU-first, single-container "lite" profile on top (sandboxed agent shell,
hybrid memory, hardware-aware model recommendations, resource budgets); see
[LITE_NOTES.md](LITE_NOTES.md) for the full divergence list and
[CHANGELOG.md](CHANGELOG.md) for what changed.

> **Upstream's own inconsistency (informational, not ours to resolve):**
> upstream's `ACKNOWLEDGMENTS.md` still contains older prose claiming the
> project's "core ships fully permissive (MIT-compatible)" — language that
> predates upstream's move to AGPL and contradicts upstream's own current
> `LICENSE` and `README.md`. We do not adopt that MIT-era prose; we go by
> upstream's actual `LICENSE` file and `README.md`, both of which say
> AGPL-3.0-or-later. Per upstream's `LICENSE` file, its `Copyright (C) <year>
> <name of author>` line is the unfilled FSF AGPL template — upstream has not
> substituted its own name/year into it, so we preserve it exactly as
> upstream ships it (verbatim FSF boilerplate) rather than guessing at a
> completion upstream itself hasn't made.
