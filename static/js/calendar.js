...
async function _fetchEvents(start, end, force) {
  // Lite mode has no calendar (CalDAV) routes registered — the various
  // background triggers (window focus, visibilitychange, calendar-refresh)
  // would otherwise hit /api/calendar/events and 404 on every page load.
  if (window.__LITE_MODE__ === true) return;
...
async function _fetchCalendars() {
  // Lite mode has no calendar routes — skip the /api/calendar/calendars fetch
  // that would 404. _calendars stays empty; the calendar UI is gone in lite.
  if (window.__LITE_MODE__ === true) { _calendars = []; return; }
