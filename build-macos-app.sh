...
APP_NAME="Odysseus Lite"
INSTALL_DIR="$REPO_DIR"
PORT="${ODYSSEUS_PORT:-7860}"
DIST="$REPO_DIR/dist"
APP="$DIST/$APP_NAME.app"

echo "Building Odysseus Lite.app"
...
<plist version="1.0">
<dict>
    <key>CFBundleName</key>            <string>Odysseus Lite</string>
    <key>CFBundleDisplayName</key>     <string>Odysseus Lite</string>
...
#!/bin/bash
# Odysseus Lite.app — start the local server and open the UI in an app window.
...
notify() { /usr/bin/osascript -e "display notification \"$1\" with title \"Odysseus Lite\"" >/dev/null 2>&1; }
die_gui() {
  /usr/bin/osascript -e "display dialog \"$1\" with title \"Odysseus Lite\" buttons {\"OK\"} default button 1 with icon stop" >/dev/null 2>&1
  exit 1
}

[ -x "$UVICORN" ] || die_gui "Odysseus Lite isn't set up yet. Open Terminal and run:
...
for i in $(seq 1 120); do
  ...
  kill -0 "$SERVER_PID" 2>/dev/null || die_gui "Odysseus Lite failed to start. Log:
$LOG"
  sleep 1
done
...
if [ "$READY" = "1" ]; then
  ...
  notify "Odysseus Lite is taking a while — open $URL once it finishes starting."
fi
wait "$SERVER_PID"
LAUNCHER
...
echo "Packaging dist/Odysseus Lite.dmg"
STAGE="$(mktemp -d)/dmg"
mkdir -p "$STAGE"
cp -R "$APP" "$STAGE/"
ln -s /Applications "$STAGE/Applications"
rm -f "$DIST/$APP_NAME.dmg"
hdiutil create -volname "Odysseus Lite" -srcfolder "$STAGE" -ov -format UDZO "$DIST/Odysseus Lite.dmg" >/dev/null
rm -rf "$STAGE"

echo ""
echo "Done:"
echo "  $APP"
echo "  $DIST/Odysseus Lite.dmg"
echo ""
echo "Run it:        open '$APP'"
echo "Install:       open '$DIST/Odysseus Lite.dmg'  (drag Odysseus Lite to Applications)"
