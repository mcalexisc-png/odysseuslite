...
MEMORY_BACKEND = os.getenv("MEMORY_BACKEND", "sqlite_fts")
SEARCH_PROVIDER = os.getenv("SEARCH_PROVIDER", "duckduckgo")
CHROMADB_ENABLED = os.getenv("CHROMADB_ENABLED", "false").lower() == "true"
RESEARCH_MAX_STEPS = int(os.getenv("RESEARCH_MAX_STEPS", "5"))
RESEARCH_CONCURRENCY = int(os.getenv("RESEARCH_CONCURRENCY", "1"))
...
_TIMEOUT_EXEMPT_PREFIXES = (
    ...
    "/api/memory/audit",    # LLM dedup pass over all memories; duration scales with memory count
)
...
# Gallery CRUD — always registered: chat uses DELETE /api/gallery/{id} to
# clean up AI-generated images; the agent app_api / edit_image tools use
# the library/album/transform endpoints. The gallery *UI* (rail button,
# gallery.js page) is hidden in lite mode by the /api/lite-config gate.
from routes.gallery_routes import setup_gallery_routes, setup_gallery_image_editor_routes
app.include_router(setup_gallery_routes())
# Image-editor processing (/api/image/*: inpaint, harmonize, rembg, …) —
# disabled in lite: requires a GPU diffusion server or heavy PIL transforms.
if not LITE_MODE:
    app.include_router(setup_gallery_image_editor_routes())

# Persisted image-editor drafts (server-backed projects) — disabled in Lite Mode
if not LITE_MODE:
...
# Calendar (CalDAV) — disabled in Lite Mode
if not LITE_MODE:
...
# Cookbook — full upstream (model download/serve/cache, 270-model GPU scan).
# Disabled in lite: the GPU scanner, VRAM column, and engine-build UI have no
# place on a CPU-only 4GB host. Lite mode uses the Lite Cookbook instead
# (/api/lite/cookbook) which serves a curated, RAM-tiered CPU-only table.
if not LITE_MODE:
    from routes.cookbook_routes import setup_cookbook_routes
    app.include_router(setup_cookbook_routes())
    # Hardware model fitting (cookbook "What Fits?" tab — GPU-aware)
    from routes.hwfit_routes import setup_hwfit_routes
    app.include_router(setup_hwfit_routes())

# Lite Cookbook — always registered: curated CPU-only RAM-tiered recommender.
# In lite mode this is the ONLY cookbook. In full mode it adds the /api/lite/
# cookbook endpoints alongside the upstream cookbook.
...
# Contacts (CardDAV) — disabled in Lite Mode
if not LITE_MODE:
...
# Lite-mode HTML stripping lives in services/lite_html.py so it can be
# unit-tested without booting the whole app (see tests/test_lite_ui_and_mcp_gating.py).
from services.lite_html import strip_lite_html as _strip_lite_html


def _serve_html_with_nonce(request: Request, file_path: str) -> HTMLResponse:
    """Read an HTML file, inject the CSP nonce, and strip lite-mode markup."""
    with open(file_path, "r", encoding="utf-8") as f:
        html = f.read()
    nonce = getattr(request.state, "csp_nonce", "")
    html = html.replace("{{CSP_NONCE}}", nonce)
    # Expose the build mode to the frontend (window.__LITE_MODE__). Always
    # substituted so the placeholder never leaks to the browser as literal text.
    html = html.replace("{{LITE_MODE}}", "true" if LITE_MODE else "false")
    if LITE_MODE and file_path.endswith("index.html"):
        html = _strip_lite_html(html)
...
async def get_lite_config():
    return {
        "lite_mode": LITE_MODE,
        "memory_backend": MEMORY_BACKEND,
        "search_provider": SEARCH_PROVIDER,
        "chromadb_enabled": CHROMADB_ENABLED,
        "research_max_steps": RESEARCH_MAX_STEPS,
        "research_concurrency": RESEARCH_CONCURRENCY,
        "features": {
            "email": not LITE_MODE,
            "calendar": not LITE_MODE,
            "image_editor": not LITE_MODE,
            "theme_editor": not LITE_MODE,
            "chromadb_memory": CHROMADB_ENABLED,
        }
    }
...
async def _startup_event():
    ...
    # Lite overlay: announce the active agent-shell sandbox mode (and warn loudly
    # if the operator has disabled the sandbox via AGENT_SHELL_MODE=unrestricted).
    try:
        from services.shell.sandbox import log_startup_mode as _log_shell_mode
        _log_shell_mode()
    except Exception as _e:
        logger.debug(f"Shell sandbox startup log skipped: {_e}")
