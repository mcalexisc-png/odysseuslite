...
class AuthManager:
    ...
    def admin_reset_password(self, username: str, new_password: str) -> bool:
        """Set a user's password WITHOUT the current password.

        Operator-only recovery path (used by the `reset-password` CLI in
        setup.py) so a lost admin password doesn't require wiping the database.
        Has no auth gate of its own — it must only ever be reachable from a
        local operator with filesystem access to data/auth.json, never an HTTP
        route. Optionally clears 2FA so a locked-out admin can fully recover.
        """
        username = (username or "").strip().lower()
