...
def setup_gallery_image_editor_routes() -> APIRouter:
    """Image-processing endpoints for the gallery editor (GPU/PIL-heavy).

    Registered separately from setup_gallery_routes() so lite mode can omit
    them without affecting gallery CRUD (used by chat and the agent).
    Endpoints: /api/image/inpaint, /harmonize, /sharpen, /denoise,
    /upscale-local, /remove-bg, /enhance-face.
    """
    router = APIRouter(tags=["gallery-editor"])
