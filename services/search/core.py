...
def _lite_default_provider() -> Optional[str]:
    """Odysseus Lite: honor the SEARCH_PROVIDER env var as the effective default.

    Upstream reads only admin settings, which default to `searxng` — but lite
    ships no SearXNG. On a fresh lite box (no admin search config) this returns
    the env value (default `duckduckgo`) so search "just works" with no key.
    Returns None when admin settings already pin a provider (admin wins).
    """
    import os
    settings = _get_search_settings()
    if settings.get("search_provider"):
        return None  # admin-configured provider takes precedence
    env = (os.getenv("SEARCH_PROVIDER") or "").strip().lower()
    return env or None


def _effective_primary(settings_provider: str) -> str:
    """Resolve the primary provider, letting the lite env default win on fresh boxes."""
    return _lite_default_provider() or settings_provider


def _build_provider_chain(primary: str) -> List[str]:
    """Build ordered list: primary first, then configured/default fallbacks.

    Fallback order precedence: admin `search_fallback_chain` > env
    `SEARCH_FALLBACK_ORDER` (lite) > built-in `_FALLBACK_ORDER`.
    """
    import os
    chain = [primary]
    settings = _get_search_settings()
    ...
    if not user_chain:
        env_chain = (os.getenv("SEARCH_FALLBACK_ORDER") or "").strip()
        if env_chain:
            user_chain = [s.strip() for s in env_chain.split(",") if s.strip()]
...
def searxng_search_results(query: str, count: int = 10, time_filter: str = None) -> list[dict]:
    ...
    settings = _get_search_settings()
    search_provider = _effective_primary(settings.get("search_provider", "searxng"))
        ...
        for attempt in range(2):
            try:
                ...
                if results:
                    ...
                    # Note which provider actually served — at info on the first
                    # provider, at warning when a fallback had to step in, so a
                    # rate-limited primary is visible without DEBUG logging.
                    if provider_name == provider_chain[0]:
                        logger.info(f"{provider_name} search succeeded with {len(results)} results")
                    else:
                        logger.warning(
                            f"search fell back to {provider_name} (primary "
                            f"'{provider_chain[0]}' returned nothing); "
                            f"{len(results)} results"
                        )
                    break
            except (NetworkError, ParseError, RateLimitError) as e:
                last_errors.append(f"{provider_name}: {e}")
    ...
    if not success:
        import os
        detail = "; ".join(last_errors[-3:]) if last_errors else "no results from any provider"
        logger.error(
            "All search providers failed for query %r (chain=%s): %s",
            query, provider_chain, detail,
        )
        # Odysseus Lite: don't fail silently. Surface a single error result the
        # agent/UI can show, instead of an empty list that looks like "no
        # matches". Gated so non-lite callers keep the historical [] contract.
        if (os.getenv("LITE_MODE", "").strip().lower() == "true"):
            return [{
                "title": "Web search unavailable",
                "url": "",
                "snippet": (
                    f"All search providers failed (tried: {', '.join(provider_chain)}). "
                    f"{detail}. Check connectivity or configure a keyed provider "
                    f"(SEARCH_PROVIDER / *_API_KEY). This is an error, not 'no results'."
                ),
                "error": True,
