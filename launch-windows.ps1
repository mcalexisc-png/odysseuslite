...
# 3. Install / update dependencies (prefer requirements.lite.txt for Odysseus Lite)
Write-Step "Installing dependencies (first run can take a few minutes)"
& $venvPy -m pip install --upgrade pip --quiet
if (Test-Path "requirements.lite.txt") {
    Write-Host "Installing Odysseus Lite dependencies..."
    & $venvPy -m pip install -r requirements.lite.txt
} else {
...
# 6. Set Lite mode environment variables
$env:LITE_MODE = "true"
$env:SEARCH_PROVIDER = "duckduckgo"
$env:CHROMADB_ENABLED = "false"
$env:MEMORY_BACKEND = "sqlite_fts"

# 7. Start the server (use `python -m uvicorn` - bare `uvicorn` may not be on PATH)
Write-Step ("Starting Odysseus Lite at http://{0}:{1}" -f $BindHost, $Port)
