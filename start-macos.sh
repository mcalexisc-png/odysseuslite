...
echo "▶ Odysseus Lite quick start for macOS"
...
if [ -f requirements.lite.txt ]; then
  ...
  echo "▶ Using Odysseus Lite requirements (requirements.lite.txt)"
fi
...
# In Lite mode, skip ChromaDB entirely
if [ -z "${LITE_MODE:-}" ]; then
...
# In Lite mode, skip ChromaDB entirely (SQLite FTS5 backend is used instead)
export LITE_MODE="${LITE_MODE:-true}"
export SEARCH_PROVIDER="${SEARCH_PROVIDER:-duckduckgo}"
export CHROMADB_ENABLED="${CHROMADB_ENABLED:-false}"
export MEMORY_BACKEND="${MEMORY_BACKEND:-sqlite_fts}"
...
if [ "$LITE_MODE" != "true" ]; then
  ...
  echo "▶ Lite mode: ChromaDB disabled, using SQLite FTS5 memory backend."
fi
    ...
    (
        for _ in $(seq 1 90); do
            if (exec 3<>"/dev/tcp/$PROBE_HOST/$PORT") 2>/dev/null; then
                printf '\n'
                printf '  ┌────────────────────────────────────────────┐\n'
                  printf '  │  ✓ Odysseus Lite is ready — opening your browser  │\n'
                printf '  │     %-40s │\n' "$URL"
...
echo
echo "▶ Starting Odysseus Lite — it will open in your browser at $URL"
