...
#
# Email is excluded in lite mode: the HTTP /api/email/* routes are not
# registered (app.py) and the MCP tool surface must match — exposing IMAP/SMTP
# agent tools on a host that has no email routes configured would be misleading
# and creates an unnecessary capability surface on a CPU-only 4GB box.
