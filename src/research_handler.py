...
class ResearchHandler:
    ...
    async def call_research_service(
        ...
        try:
            ...
            # Odysseus Lite: enforce the RESEARCH_MAX_STEPS / RESEARCH_CONCURRENCY
            # env caps. Upstream defined these in app.py but never wired them to
            # the researcher, so a long run could exceed the lite budget. The env
            # value is a *ceiling*: it only ever lowers max_rounds/concurrency.
            import os as _os
            _env_steps = _os.getenv("RESEARCH_MAX_STEPS")
            if _env_steps and _env_steps.strip():
                try:
                    max_rounds = min(max_rounds, max(1, int(_env_steps)))
                except (TypeError, ValueError):
                    pass
            _env_conc = _os.getenv("RESEARCH_CONCURRENCY")
            if _env_conc and _env_conc.strip():
                try:
                    _extraction_concurrency = min(_extraction_concurrency, max(1, int(_env_conc)))
                except (TypeError, ValueError):
                    pass
            ...
            researcher = DeepResearcher(
                ...
                llm_headers=llm_headers,
                max_rounds=max_rounds,
                min_rounds=max(1, min(max(2, max_rounds - 2), max_rounds)),
                max_time=max_time,
