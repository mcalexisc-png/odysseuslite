...
# ---------------------------------------------------------------------------
# Odysseus Lite: cap the agent loop via AGENT_MAX_STEPS so a runaway loop can't
# burn CPU/tokens on a low-end host. Default stays at the upstream 50 when the
# env var is unset, but docker-compose.lite.yml ships a low default (12). The
# loop already enforces this cap (src/agent_loop.py emits "rounds_exhausted").
import os as _os


def _max_agent_rounds() -> int:
    raw = _os.getenv("AGENT_MAX_STEPS")
...
MAX_AGENT_ROUNDS = _max_agent_rounds()
SHELL_TIMEOUT = 60
PYTHON_TIMEOUT = 30
