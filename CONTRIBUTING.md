...
Always check what you're about to include — `data/`, `.env`, logs, databases,
uploads, and auth/session files must never be committed:
...
If `git status --short` shows anything under `data/`, `logs/`, `uploads/`, or a
`.env` file, stop and fix `.gitignore` before committing. See the publishing
checklist in [`SECURITY.md`](SECURITY.md#publishing-a-fork).
