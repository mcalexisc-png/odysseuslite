...
- Keep Ollama, vLLM, llama.cpp, databases, and raw model/provider APIs internal-only.
...
- The agent shell/python tools are **sandboxed by default** (`AGENT_SHELL_MODE=sandboxed`): workspace-confined, no network egress (`AGENT_SHELL_NET=false`), CPU/memory rlimits, and a 120s timeout. For maximum safety on shared machines, install `bubblewrap` (`bwrap`) so an OS sandbox is used instead of the degraded fallback. Only set `AGENT_SHELL_MODE=unrestricted` (which also requires `AGENT_SHELL_ACK_UNSAFE=true`) on a trusted, isolated host. See the Agent Shell Sandbox section in `THREAT_MODEL.md`.
...
- Common internal-only ports are Odysseus Lite `7000`, Ollama `11434`, and local model/provider APIs such as `8000-8020`.
- Odysseus Lite runs as a single container — SearXNG, ChromaDB, and ntfy are not included and do not need to be secured separately.
